...
final class GeneratorFidelityHarness {
  ...
  File get manifestFile => File(
        p.join(workspaceRoot.path, 'packages/react_web/lib/src/generated/emitted_manifest.json'),
      );

  File get reportFile => File(
        p.join(workspaceRoot.path, 'packages/react_web/lib/src/generated/completeness_report.json'),
      );

  File get hostTypesFile => File(
        p.join(workspaceRoot.path, 'packages/react_codegen/lib/src/generated/web_host_types.g.dart'),
      );
  ...
  void assertManifestComplete() {
    if (!manifestFile.existsSync()) {
      throw StateError('Missing ${manifestFile.path} — run generate_factories first');
    }
    final report = _readJson(reportFile);
    final defs = report['definitions'] as Map;
    final members = report['members'] as Map;
    if (defs['dropped'] != 0) {
      throw StateError('dropped definitions ${defs['dropped']}: ${defs['missing']}');
    }
    if (members['dropped'] != 0) {
      throw StateError('dropped members ${members['dropped']}: ${members['missing']}');
    }
  ...
  void assertDictionaryValues() {
    for (final spec in ['html.dart', 'fetch.dart', 'dom.dart']) {
      final f = File(p.join(workspaceRoot.path, 'packages/react_web/lib/src/generated/web/$spec'));
  ...
  void assertHostTypes() {
    ...
    for (final name in ['Storage', 'BroadcastChannel', 'FileReader', 'Blob', 'MessageEvent']) {
  ...
  void assertNamespaceDispatch() {
    final f = File(p.join(workspaceRoot.path, 'packages/react_web/lib/src/generated/web/css_animations.dart'));
    if (!f.existsSync()) return;
    final content = f.readAsStringSync();
    // CSS etc. should go via WebRuntime, not throw
    if (content.contains('throw UnsupportedError') && content.contains('class CSS')) {
      // Check that CSS file still has throw — failure
      final cssFile = File(p.join(workspaceRoot.path, 'packages/react_web/lib/src/generated/web/cssom.dart'));
