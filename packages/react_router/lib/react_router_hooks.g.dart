...
/// Typed JS interop extension for `PathMatch` hook return values.
///
/// Direct property access on the raw JS object avoids the
/// generic [[key, value]] pairs conversion used by
/// `_pairsMap` (removed in this generation).
...
/// Value class for `PathMatch` (decoded from the hook shim).
///
...
final class PathMatch {
  PathMatch._(this._value, {
    required this.params,
    required this.pathname,
    required this.pathnameBase,
    required this.pattern,
  });

  final _PathMatchJs _value;
  ...
  factory PathMatch.fromJs(JSObject js) {
    ...
    return PathMatch._(v,
      params: _decodePairs(v.params as JSArray),
      pathname: (v.pathname as JSString).toDart,
      pathnameBase: (v.pathnameBase as JSString).toDart,
      pattern: PathPattern.fromJs(v.pattern as JSObject),
    );
  }

}

/// Typed JS interop extension for `PathPattern` hook return values.
///
/// Direct property access on the raw JS object avoids the
/// generic [[key, value]] pairs conversion used by
/// `_pairsMap` (removed in this generation).
...
final class PathPattern {
  ...
  factory PathPattern.fromJs(JSObject js) {
    ...
    return PathPattern._(v,
      path: Path.fromJs(v.path as JSObject),
      caseSensitive: v.caseSensitive == null ? null : (v.caseSensitive as JSBoolean).toDart,
      end: v.end == null ? null : (v.end as JSBoolean).toDart,
    );
  }
