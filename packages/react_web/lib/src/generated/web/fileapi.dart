...
abstract interface class Blob {
  factory Blob([List<BlobPart>? blobParts, BlobPropertyBag? options]) =>
      WebRuntime.current.createWebObject<Blob>('Blob', [blobParts, options]);
  int get size;
  String get type_;
...
final class BlobPropertyBagValue implements BlobPropertyBag {
  ...
  BlobPropertyBagValue({this.type_, this.endings});
}
...
abstract interface class File {
  factory File(
    List<BlobPart> fileBits,
    String fileName, [
    FilePropertyBag? options,
  ]) => WebRuntime.current.createWebObject<File>('File', [
    fileBits,
    fileName,
    options,
  ]);
...
final class FilePropertyBagValue implements FilePropertyBag {
  ...
  FilePropertyBagValue({this.lastModified});
}

abstract interface class FileReader {
  factory FileReader() =>
      WebRuntime.current.createWebObject<FileReader>('FileReader', []);
...
abstract interface class FileReaderSync {
  factory FileReaderSync() =>
      WebRuntime.current.createWebObject<FileReaderSync>('FileReaderSync', []);
...
abstract interface class URL {
  factory URL(String url, [String? base]) =>
      WebRuntime.current.createWebObject<URL>('URL', [url, base]);
  String get href;
