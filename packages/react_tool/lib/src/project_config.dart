...
/// One declarative TypeScript binding group (`react.yaml `js.bind`): the
/// types of [specifier] are extracted and emitted as Dart bindings.
///
/// When [names] is empty the extractor discovers the package's exported
/// components and hooks automatically, filtering out exports whose return
/// type is not a React node.
final class ReactJsBindGroup {
  final String specifier;

  /// Optional explicit name list; empty means "discover".
  ...
  /// Prefix for generated type names, avoiding collisions when two groups
  /// extract overlapping types (e.g. `Server` for a subpath module).
  final String? typePrefix;

  /// Public names to drop from discovery results (rare: symbols whose
  /// types the extractor cannot classify).
  final List<String> exclude;

  const ReactJsBindGroup({
...
final class ReactProjectConfig {
  ...
  /// Declarative TypeScript binding groups (`react.yaml `js.bind`),
  /// regenerated by `react ts bind` with no arguments.
  final List<ReactJsBindGroup> jsBindGroups;
  ...
  factory ReactProjectConfig.load([Directory? directory]) {
    ...
    final jsBindGroups = _jsBindGroups(_map(explicit['js'])['bind']);
...
/// Parses the `react.js.bind` list into declarative binding groups.
List<ReactJsBindGroup> _jsBindGroups(Object? value) {
  if (value is! List) return const [];
  final groups = <ReactJsBindGroup>[];
  for (final item in value) {
    final map = _map(item);
    final specifier = _string(map['specifier']);
    if (specifier == null) continue;
    groups.add(
      ReactJsBindGroup(
        specifier: specifier,
        names: _stringList(map['names']),
        output: _string(map['output']),
        shim: _string(map['shim']),
        hooks: _string(map['hooks']),
        namespace: _string(map['namespace']),
        prefix: _string(map['prefix']),
        typePrefix: _string(map['typePrefix']),
        exclude: _stringList(map['exclude']),
      ),
    );
  }
  return groups;
