...
final class JsBridgeEmitter {
  ...
  String _componentSource(ReactLibraryModel model, ReactComponentModel component) {
    final toJSBuffer = StringBuffer();
    final fromJSBuffer = StringBuffer();
    final registrationBuffer = StringBuffer();

    toJSBuffer.writeln('JSObject _${component.name}_toJS(${_recordCode(component.propsRecord)} props) {');
    ...
    for (final prop in component.props) {
      if (_isCallback(prop.type)) {
        ...
        toJSBuffer.writeln("  o.setProperty('${prop.name}'.toJS, ${prop.required ? 'callbackToJS($modelCode)' : 'props.${prop.name} == null ? null : callbackToJS($modelCode)'});");
      } else if (_isNullable(prop.type)) {
        toJSBuffer.writeln("  if (props.${prop.name} != null) o.setProperty('${prop.name}'.toJS, props.${prop.name}!.toJS);");
      } else {
        toJSBuffer.writeln("  o.setProperty('${prop.name}'.toJS, props.${prop.name}.toJS);");
      }
    }
    ...
    toJSBuffer.writeln('}');
        ...
        fromJSBuffer.writeln('${_recordCode(component.propsRecord)} _${component.name}_fromJS(JSObject js) {');
    for (final prop in component.props) {
      if (_isCallback(prop.type)) {
    ...
    final propsReturn = component.props.map((prop) => '${prop.name}: ${prop.name}').join(', ');
    ...
    registrationBuffer.writeln('    final dartProps = _${component.name}_fromJS(props);');
    registrationBuffer.writeln('    return toReactJS(impl.${component.name}(dartProps));');
    ...
    registrationBuffer.writeln("  ReactRegistry.register(id${component.name}.value, \$${component.name},");
    registrationBuffer.writeln("      toJS: (p) => _${component.name}_toJS(p as ${_recordCode(component.propsRecord)}),");
    registrationBuffer.writeln("      fromJS: (js) => _${component.name}_fromJS(js));");
    registrationBuffer.writeln('}');
  ...
  ReactCallbackModel _callbackModel(ReactTypeRef type) {
    ...
    return ReactCallbackModel(
      positional: type.positional.map((param) => ReactCallbackParameter(
        name: param.name,
        type: param.type,
  ...
  String _accessor(ReactPropModel prop, String componentName) {
    ...
    return switch ((prop.type, prop.required)) {
      ...
      (NamedTypeRef(symbol: 'int'), true) => 'requiredJSInt(js, "${prop.name}"$nullable)',
      (NamedTypeRef(symbol: 'num'), true) || (NamedTypeRef(symbol: 'double'), true) => 'requiredJSDouble(js, "${prop.name}"$nullable)',
      (NamedTypeRef(symbol: 'bool'), true) => 'requiredJSBool(js, "${prop.name}"$nullable)',
