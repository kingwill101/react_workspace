...
`routed_react` is the integration layer between the two portable kernels:
...
- `routed_core` owns HTTP routing, middleware, providers, and portable host
  contracts.
- `routed_react` composes those contracts without changing either core package.
...
```sh
cd packages/routed_react/example
dart pub get
