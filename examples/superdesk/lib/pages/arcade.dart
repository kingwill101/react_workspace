          ...
          div(
            ...
            children: [
              select(
                ...
                onChange: (e) => props.onGame((e.target as HTMLSelectElement).value),
          ...
          children: [
            div(
              ...
              children: [
                ...
                span(
                  className: 'text-[11px] font-black bg-[#FFE8A3] border-2 border-dark rounded-full px-3 py-1',
                  children: [Text('Tap a card to hear pronunciation')],
                ),
              ],
            ),
            div(
              className: 'grid sm:grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-cream border-3 border-dark rounded-[24px]',
              children: [
                for (final card in carouselCards)
                  div(
                    key: card['es'] as String,
                    onClick: (_) => props.onToast('Selected: ${card['es']} • ${card['en']} ${card['emoji']}'),
                    className: 'border-3 border-dark rounded-[20px] p-4 flex flex-col items-center justify-center gap-2 cursor-pointer shadow-chunky hover:scale-105 transition-transform',
                    style: {'background': card['color'] as String},
                    children: [
                      span(className: 'text-[44px]', children: [Text(card['emoji'] as String)]),
                      span(className: 'font-black text-[18px]', children: [Text(card['es'] as String)]),
                      span(className: 'font-bold text-[13px] text-gray-600', children: [Text(card['en'] as String)]),
                    ],
                  ),
              ],
            ),
